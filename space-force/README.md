...
High-level overview of the Space Force game package.
...
- `space-force`: `bun run build` passes.
- `space-force`: `bun run typecheck` passes.
- `space-force/server`: `npm run typecheck` passes.
- `space-force/server`: `npm run build` passes.
...
2. Client connects to Colyseus room `space_force`.

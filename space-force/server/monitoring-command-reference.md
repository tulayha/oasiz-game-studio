# Space Force Server Monitoring Command Reference
...
```bash
pm2 logs space-force-colyseus --time --lines 0
```

### 3) Focused disconnect signal stream

```bash
pm2 logs space-force-colyseus --time --lines 0 | grep -E "SpaceForceRoom.onLeave|consented=false"
```

### 4) Critical lifecycle/error stream (tank/restart signal)

```bash
pm2 logs space-force-colyseus --time --lines 0 | grep -E "Server.lifecycle|Uncaught exception|Unhandled promise rejection|HTTP server error|SpaceForceRoom.onLeave.*consented=false"
```
...
```bash
pm2 logs space-force-colyseus --time --lines 0 | tee -a /tmp/space-force-colyseus-live.log
```
...
```bash
LOG="/tmp/space-force-correlation-$(date +%Y%m%d-%H%M%S).log"
...
while true; do
  ...
  pm2_cpu="$(echo "$pm2_json" | jq -r '.[] | select(.name=="space-force-colyseus") | (.monit.cpu // 0)' | head -n1)"
  pm2_mem_mb="$(echo "$pm2_json" | jq -r '.[] | select(.name=="space-force-colyseus") | (((.monit.memory // 0)/1048576))' | head -n1)"
  pm2_restarts="$(echo "$pm2_json" | jq -r '.[] | select(.name=="space-force-colyseus") | (.pm2_env.restart_time // 0)' | head -n1)"
...
```bash
scp space-force-droplet:/tmp/space-force-correlation-*.log .
scp space-force-droplet:/tmp/space-force-colyseus-live.log .
rg -n "1006|isServerDisconnect=true" loadtest-logs/loadtest-lobbyfill-*.log
rg -n "SpaceForceRoom.onLeave|consented=false|leftUnconsented|pm2.restarts|net.rxBps|net.txBps|rttP95" ./space-force-correlation-*.log ./space-force-colyseus-live.log
```

If you do not use SSH config alias:

```bash
scp root@<droplet-ip>:/tmp/space-force-correlation-*.log .
scp root@<droplet-ip>:/tmp/space-force-colyseus-live.log .
```

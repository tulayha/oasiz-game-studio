...
import main from "./lobbyfill.loadtest";
...
function bridgeLoadtestArgsToEnv(): void {
  const bridges: ArgBridge[] = [
    { env: "LOADTEST_USERS_PER_ROOM", names: ["usersPerRoom"] },
    { env: "LOADTEST_REQUEST_TIMEOUT_MS", names: ["requestTimeoutMs"] },
    { env: "LOADTEST_WAIT_FOR_GROUP_MS", names: ["waitForGroupMs"] },
    { env: "LOADTEST_START_DELAY_MS", names: ["startDelayMs"] },
    { env: "LOADTEST_START_FALLBACK_MS", names: ["startFallbackMs"] },
...
function resolveDefaultOutputPath(): string {
  ...
  return path.join(
    dirPath,
    "loadtest-lobbyfill-" + buildTimestampText(new Date()) + ".log",
  );
}
...
if (!helpRequested) {
  ...
  if (outputPath) {
    ...
    console.log("[LoadTest.run-lobbyfill]", "Using output log " + outputPath);
  }
}

...
4. If `space-force-demo-seen` is missing, attract overlay is shown.
5. If `space-force-demo-seen` exists, app goes straight to start menu with background demo battle (`MENU` state).
...
- `src/main.ts`
  ...
  - Attract is gated by `localStorage` key `space-force-demo-seen`.

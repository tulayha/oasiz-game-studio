# Astro Party Random Input Lag - Deep Findings Log
...
- Evidence:
  - Every sim tick calls `hooks.onSnapshot(this.buildSnapshot())`: `shared/sim/AstroPartySimulation.ts` (update loop).
...
- Evidence:
  - `AstroPartySimulation.createCollisionHandlersContext()` defines `removeProjectileEntity` as:
...
- Evidence:
  - `AstroPartySimulation.update()` allocates `previousProjectileVelocities = this.captureBodyVelocities(this.projectileBodies)` every tick.
...
- Evidence:
  ...
  - This runs in the main tick path (`AstroPartySimulation.update`) every PLAYING tick.
- Why this matches random lag:
...
- Evidence:
  - `AstroPartyRoom.broadcastSnapshotToClients(...)` loops all clients and calls `sendSnapshotToClient(client, snapshot)`.
...
- Evidence:
  - `AstroPartySimulation.update()` recreates tick-scoped context objects repeatedly:
...
- Evidence:
  ...
  - Called every PLAYING tick from `AstroPartySimulation.update()`.
- Why this matches random lag:
...
- Time found: 2026-03-02
- Area: server snapshot fanout (`server/src/rooms/AstroPartyRoom.ts`)
- Evidence:
...
- Evidence:
  - `AstroPartySimulation` still captures unused `previousProjectileVelocities`.
...
- Time found: 2026-03-02
- Area: collision-group lifecycle (`shared/sim/physics/Physics.ts`, `AstroPartySimulation.ts`)
- Evidence:

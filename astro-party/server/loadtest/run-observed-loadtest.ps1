param(
  ...
  [string]$DropletHost = "",
  [string]$RemoteObserverPath = "/root/astro-observe.sh",
  [string]$RemoteObserveRoot = "/tmp/astro-observe",
  [string]$OpsUrl = "http://127.0.0.1:2567/ops/stats",
  [string]$OpsToken = "",
  [int]$ObserveIntervalSec = 2,
  [string]$Interface = "",
  [string]$Pm2App = "astro-party-colyseus",
...
function Resolve-ObserverScriptPath {
  $cwdCandidate = Join-Path -Path "." -ChildPath "loadtest/astro-observe.sh"
  if (Test-Path -Path $cwdCandidate) {
    return $cwdCandidate
  }
  $scriptDir = Split-Path -Path $PSCommandPath -Parent
  return (Join-Path -Path $scriptDir -ChildPath "astro-observe.sh")
}
